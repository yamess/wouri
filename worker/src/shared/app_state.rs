...
impl AppState  {
    pub fn new() -> Self {
        ...
        let redis_task_repository = Arc::new(RedisTaskRepository::new(&config.redis.redis_connection_string));
        let task_actor_pool = TaskActorPool::new(config.app.task_pool_size, redis_task_repository.clone());
        Self {
            task_actor_pool,
